// This code is part of the Fungus library (http://fungusgames.com) maintained by Chris Gregan (http://twitter.com/gofungus).
// It is released for free under the MIT open source license (https://github.com/snozbot/fungus/blob/master/LICENSE)

﻿using UnityEngine;
using System.Collections;

namespace Fungus
{
    /// <summary>
    /// Do multiple say and portrait commands in a single block of text. Format is: [character] [portrait] [stage position] [: Story text].
    /// </summary>
    [CommandInfo("Narrative", 
                 "Conversation", 
                 "Do multiple say and portrait commands in a single block of text. Format is: [character] [portrait] [stage position] [: Story text]")]
    [AddComponentMenu("")]
    [ExecuteInEditMode]
    public class Conversation : Command
    {
        [Tooltip("A text file containing the conversation to play")]
        [SerializeField] protected TextAsset conversationFile;

        [TextArea(1,3)]
        [Tooltip("A short description of the conversation")]
        [SerializeField] protected string description;

        [Tooltip("A conversation to play. This is appended to the conversation in the text file, if exists")]
        [SerializeField] protected StringDataMulti conversationText;

        protected ConversationManager conversationManager = new ConversationManager();

        protected virtual void Start()
        {
            conversationManager.PopulateCharacterCache();
        }

        protected virtual string GetConversationString ()
        {
            if (conversationFile == null)
                return conversationText;
            else
                return conversationFile.text + "\n" + conversationText;
        }

        protected virtual IEnumerator DoConversation()
        {
            var flowchart = GetFlowchart();
            string subbedText = flowchart.SubstituteVariables(GetConversationString());

            yield return StartCoroutine(conversationManager.DoConversation(subbedText));

            Continue();
        }

        #region Public members

        public override void OnEnter()
        {
            StartCoroutine(DoConversation());
        }

        public override string GetSummary()
        {
            if (description == null || description == "")
                return conversationText.Value;
            else
                return description;
        }

        public override Color GetButtonColor()
        {
            return new Color32(184, 210, 235, 255);
        }

        #endregion
    }
}